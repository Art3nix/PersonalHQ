...
class BucketExperience(db.Model):  # pylint: disable=R0903; # sqlalchemy class used to only store data
    """Association class for Many-to-Many relationship between TimeBuckets and Experiences"""
    __tablename__ = 'bucket_experiences'
    bucket_id: Mapped[int] = mapped_column(ForeignKey("time_buckets.id"), primary_key=True)
    experience_id: Mapped[int] = mapped_column(ForeignKey("experiences.id"), primary_key=True)

    bucket = db.relationship('TimeBucket', back_populates='experiences')
